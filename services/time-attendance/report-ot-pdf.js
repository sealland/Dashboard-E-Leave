...
const REPORT_BASE = "https://reports.zubbsteel.com/pdfEmployeeOT.php";

function buildReportUrl({ period, dfCode, brCode, c }) {
  const url = new URL(REPORT_BASE);
  url.searchParams.set("period", period);
  url.searchParams.set("df_code", dfCode);
...
function initUi(auth) {
  ...
  const dfSelect = document.getElementById("df_code");
  ...
  if (params.get("df_code")) dfSelect.value = params.get("df_code");
  ...
  function refresh() {
    ...
    const dfCode = dfSelect.value;
    const brCode = brSelect.value;
    if (!period || !dfCode) {
      setFrameSrc("about:blank");
      return;
    }
    const src = buildReportUrl({ period, dfCode, brCode, c });
    setFrameSrc(src);
    const u = new URL(window.location.href);
    u.searchParams.set("period", period);
    u.searchParams.set("df_code", dfCode);
